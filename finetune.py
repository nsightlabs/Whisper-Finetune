...
add_arg("metric_for_best_model",            type=str,  default='loss',  help="只保存最新检查点的数量")
add_arg("report_to",            type=str,  default='none',  help="只保存最新检查点的数量")
add_arg("run_name",            type=str,  default='',  help="只保存最新检查点的数量")
add_arg("max_steps",            type=int,  default=-1,  help="最大训练步数")
add_arg("include_metrics", type="store_true", default=False, help="是否在训练时计算评估指标")
...
def main():
    ...
    training_args = \
        Seq2SeqTrainingArguments(output_dir=output_dir,  # 保存检查点和意志的目录
                                 ...
                                 eval_strategy=("steps" if args.eval_steps > 0 else "no"),  # 指定按照步数评估模型
                                 # load_best_model_at_end=True,  # 指定是否在结束时加载最优模型
                                 fp16=args.fp16,  # 是否使用半精度训练
                                 report_to=args.report_to,  # 指定使用tensorboard保存log
    ...
    trainer = Seq2SeqTrainer(args=training_args,
                             ...
                             compute_metrics=(partial(compute_metrics, processor=processor) if args.include_metrics else None)
                             )
